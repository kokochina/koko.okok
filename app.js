...
function exportLearningData(){const payload={app:'koko.okok',version:1,exportedAt:new Date().toISOString(),progress:state.progress};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`koko-okok-learning-data-${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Learning data exported')}
function mergeImportedProgress(incoming){const merged={...state.progress};Object.entries(incoming).forEach(([id,record])=>{if(!record||typeof record!=='object')return;const current=merged[id];if(id==='__settings'){if(!current||(record.last||0)>=(current.last||0))merged[id]={...(current||{}),...record};return}const incomingTime=record.last||record.due||0,currentTime=current?.last||current?.due||0;if(!current||incomingTime>=currentTime)merged[id]=record});return merged}
async function importLearningData(file){try{const parsed=JSON.parse(await file.text()),incoming=parsed?.progress||parsed;if(!incoming||typeof incoming!=='object'||Array.isArray(incoming))throw new Error('Invalid data');state.progress=mergeImportedProgress(incoming);saveProgress();const savedWordId=state.progress.__settings?.lastWordId,savedIndex=state.words.findIndex(word=>String(word.id)===String(savedWordId));if(savedIndex>=0)state.current=savedIndex;renderWord(false);updateStats();document.querySelector('.sidebar').classList.remove('open');toast('Learning data imported and merged')}catch{toast('This learning data file is not valid')}finally{$('importProgressInput').value=''}}
...
function renderWord(autoPlay=true){
  ...
  const now=Date.now();recordFor(word.id).last=now;state.progress.__settings={...(state.progress.__settings||{}),lastWordId:word.id,last:now};saveProgress();window.scrollTo({top:0,behavior:'smooth'});if(autoPlay&&!isMobileDevice())testAudioTimer=setTimeout(()=>{testAudioTimer=null;if(state.view==='learn'&&currentWord()?.id===word.id)speakWord(word)},120);
}
...
function isMobileDevice(){return /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1)||(navigator.maxTouchPoints>1&&window.innerWidth<1200)}
...
function speak(text,rate=state.audioRate){if(!text)return;if(!('speechSynthesis'in window)){toast('Audio is unavailable here. Open this site in Safari, Edge, or Chrome.');return}claimAudio();speechSynthesis.resume();const generation=audioGeneration,started=Date.now(),play=()=>{if(generation!==audioGeneration)return;const voice=preferredVoice(),voicesReady=speechSynthesis.getVoices().length>0;if(!voicesReady&&Date.now()-started<1800){setTimeout(play,150);return}const u=new SpeechSynthesisUtterance(text);u.lang='en-US';u.rate=rate;if(voice)u.voice=voice;activeUtterance=u;u.onend=()=>{if(generation===audioGeneration)activeUtterance=null};u.onerror=()=>{if(generation===audioGeneration){activeUtterance=null;toast('Audio could not start. Tap play again or open the site in Safari, Edge, or Chrome.')}};speechSynthesis.speak(u)};play()}
...
async function init(){
  ...
  $('exportProgressButton').onclick=exportLearningData;$('importProgressButton').onclick=()=>$('importProgressInput').click();$('importProgressInput').onchange=event=>{const file=event.target.files?.[0];if(file)importLearningData(file)};
