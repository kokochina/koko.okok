...
function speak(text,rate=state.audioRate){if(!text||!('speechSynthesis'in window))return;claimAudio();const generation=audioGeneration,started=Date.now(),play=()=>{if(generation!==audioGeneration)return;const voice=andrewVoice();if(!voice){if(Date.now()-started<4000){setTimeout(play,150);return}toast('Andrew voice is unavailable in this browser.');return}const u=new SpeechSynthesisUtterance(text);u.lang='en-US';u.rate=rate;u.voice=voice;activeUtterance=u;u.onend=()=>{if(generation===audioGeneration)activeUtterance=null};u.onerror=()=>{if(generation===audioGeneration)activeUtterance=null};speechSynthesis.speak(u)};play()}
...
function playTestAudio(word){if(!('speechSynthesis'in window))return;claimAudio();const generation=audioGeneration,exampleIndex=word.senses.findIndex(s=>s.example),items=[`${cleanHeadword(word)}.`];if(exampleIndex>=0)items.push(word.senses[exampleIndex].example);const started=Date.now(),begin=()=>{if(generation!==audioGeneration)return;const voice=andrewVoice();if(!voice){if(Date.now()-started<4000){setTimeout(begin,150);return}toast('Andrew voice is unavailable in this browser.');return}const playAt=index=>{if(index>=items.length||generation!==audioGeneration)return;const u=new SpeechSynthesisUtterance(items[index]);u.lang='en-US';u.rate=index===0?Math.min(state.audioRate,0.88):state.audioRate;u.voice=voice;activeUtterance=u;u.onend=()=>{if(generation!==audioGeneration)return;activeUtterance=null;playAt(index+1)};u.onerror=()=>{if(generation!==audioGeneration)return;activeUtterance=null};speechSynthesis.speak(u)};playAt(0)};begin()}
