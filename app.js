...
function andrewVoice(){const voices=speechSynthesis.getVoices(),american=voices.filter(v=>/^en-US$/i.test(v.lang)),english=voices.filter(v=>/^en(?:-|_)/i.test(v.lang));return voices.find(v=>/^Microsoft Andrew Online \(Natural\)/i.test(v.name)||/AndrewNeural/i.test(v.name))||voices.find(v=>/Andrew/i.test(v.name)&&!/Multilingual/i.test(v.name))||american.find(v=>/Christopher|Guy|David|Eric|Mark|Roger|Alex|Nathan|Matthew|Aaron|Fred|Ralph|Daniel|Male/i.test(v.name))||english.find(v=>/Christopher|Guy|David|Eric|Mark|Roger|Alex|Nathan|Matthew|Aaron|Fred|Ralph|Daniel|Male/i.test(v.name))||american[0]||english[0]||null}
